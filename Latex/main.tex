...
Prior hardware-aware methods~\citep{halp2021,haloc2023} treat latency as a black-box, applying solutions to specific architectures such as CNNs without isolating root causes or providing parameter-budget guarantees.
...
\begin{itemize}
    ...
    \item We conduct a systematic full-stack analysis tracing root causes at the framework, library, and hardware levels (\S\ref{sec:analysis}).
    \item We formalize dimension selection as a constrained optimization and provide a dynamic programming solver (\S\ref{sec:gac}).
...
We evaluate on Llama-3-8B with $\rho=15\%$, using PyTorch~2.9.1, CUDA~12.8, and FP16. Experiments were conducted on two GPUs: NVIDIA Ampere (A100-80GB) and Hopper (H100-80GB). Unless stated otherwise, the results are collected from A100.
