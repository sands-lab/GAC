...
For example, ASVD-compressed~\citep{asvd} Llama-3-8B~\citep{llama3} has 15\% fewer parameters yet runs no faster than the uncompressed baseline, because 95\% of its dimensions are misaligned.
...
We evaluate GAC on Llama-3-8B with ASVD~\citep{asvd} and LLM-Pruner~\citep{llmpruner}, achieving 100\% alignment and recovering up to 1.5$\times$ speedup while preserving model quality.
\end{abstract}
