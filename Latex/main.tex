...
value decomposition (AVSD) has 15\% fewer parameters yet runs no faster than the uncompressed baseline, because 95\% of its dimensions are misaligned.
...
Pretrained model comes with carefully designed dimensions that are GPU-friendly, such as 8-aligned dimensions for FlashAttention~\cite{flashattention}.
...
\textbf{(4)}~Preliminary evaluation on Llama-3-8B~\cite{llama3} achieves up to 1.5$\times$ speedup without sacrificing accuracy (\S\ref{sec:eval}).
...
Second, $M$ and $N$ show \emph{kernel-switching cliffs}: at certain boundaries (e.g., $M{=}1728 \to 1729$), latency jumps (e.g., $\sim$30\%), where cuBLAS heuristic selected an inefficient kernel.
