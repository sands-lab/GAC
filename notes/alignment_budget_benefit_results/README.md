...
- `palu_a100_results/operator_profile_issue35/`: repo-tracked landing zone for direct baseline / PaLU / aligned GAC operator-profiler snapshots, including the publish script contract and the expected summary / provenance layout.
...
The `palu_a100_results/operator_profile_issue35/` directory is the direct-profiler follow-up to that structural split. It is where baseline / PaLU / aligned GAC `prefill` and `decode` snapshots can be published as a structured operator-family report instead of staying as ad hoc profiler dumps.
