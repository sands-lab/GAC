...
- The current code-path prototype for that prefill follow-up now lives in `prefill_dispatch_reduction.md` and `prefill_dispatch_reduction_summary.json`; it verifies grouped reconstruction dispatch reduction and padding overhead, but is not a measured GPU speedup.
